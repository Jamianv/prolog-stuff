...
decBound(In, Out):-
    ...
    Out is In - 1.
interpret(_, _,true):-!.
interpret(Bound, Call,(A, B)):-
    !,
    interpret(Bound,Call, A),
    interpret(Bound,Call, B).
interpret(Bound, [Name/Arity], Call):-
    functor(Call, CallName, _),
    CallName == Name,
    decBound(Bound, NewBound),
    clause(Call, Body),
    interpret(NewBound, [Name/Arity], Body).
interpret(Bound, [Name/Arity], Call):-
    functor(Call, CallName, _),
    CallName \= Name,
    clause(Call, Body),
    interpret(Bound, [Name/Arity], Body).
